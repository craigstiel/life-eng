  ...
  <v-app id="eng-home">
    <v-app-bar app color="#9ebc9a" flat>
      <v-container v-if="$auth.check()" class="py-0 fill-height">
        ...
        <v-spacer></v-spacer>
        <v-btn text @click.prevent="$auth.logout()">Выход</v-btn>
      </v-container>
    ...
    <v-main class="second-color">
      <v-container>
        <v-row>
          ...
          <v-col>
            <v-sheet v-if="$auth.check()" min-height="70vh" rounded="lg">
              <router-view></router-view>
            </v-sheet>
            <v-sheet v-else max-width="60vh" min-height="30vh" rounded="lg" class="mg-register mg-t-register">
              <router-view></router-view>
            </v-sheet>
          </v-col>
        </v-row>
